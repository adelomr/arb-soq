                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                             ...
                             <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                 {categories.map((category) => {
                                     ...
                                     const href = getPageUrlForCategory ? getPageUrlForCategory(category.id, undefined, catName) : (category.id === 'stores' ? '/shops' : `/search?q=${encodeURIComponent(catName)}`);
                                     return (
                                        <Link key={category.id} href={href} className="w-full">
                                           <Card className="flex flex-col items-center justify-center p-4 h-28 text-center transition-all hover:bg-primary/5 hover:shadow-md hover:-translate-y-1">
                                                ...
                                                <p className="text-sm font-semibold text-center w-full truncate">{catName}</p>
                                           </Card>
                                        </Link>
                                     )
                                })}
